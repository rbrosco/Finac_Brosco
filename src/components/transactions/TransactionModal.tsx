...
export default function TransactionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="glass-panel w-full max-w-lg rounded-t-3xl sm:rounded-2xl border border-slate-700/80 shadow-2xl p-5 sm:p-6 relative max-h-[90vh] overflow-y-auto">
        <div className="w-12 h-1.5 bg-slate-700 rounded-full mx-auto mb-3 opacity-60 sm:hidden" />

        <button
          onClick={onClose}
          className="absolute top-4 right-4 sm:top-5 sm:right-5 text-slate-400 hover:text-white p-1.5 rounded-xl hover:bg-slate-800 bg-slate-900/60 border border-slate-800"
        >
          <X className="w-5 h-5" />
        </button>
